...
const API_PDF_PATTERN = /^\/leads\/\d+\/strategy-brief\.pdf$|^\/leads\/\d+\/strategy-deck[^"]*\.pptx$/;

function downloadApiFile(path: string) {
    ...
    <>
      {parts.map((part, i) => {
        ...
        if (linkMatch) {
          ...
          if (API_PDF_PATTERN.test(href)) {
            return (
              <button
                key={i}
                type="button"
                onClick={() => downloadApiFile(href)}
                className="text-violet-600 underline underline-offset-2 hover:text-violet-800 transition-colors cursor-pointer"
              >
                {linkMatch[1]}
              </button>
            );
