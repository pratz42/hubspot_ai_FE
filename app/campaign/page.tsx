...
export default function CampaignPage() {
  ...
  const handleLsAnalyze = async (assetId: number) => {
    ...
    try {
      ...
      setLsAnalysis((prev) => ({ ...prev, [assetId]: { analysis: { performance_summary: "Analysis failed. Please try again." } } as LinkedInSocialAnalysis }));
    } finally {
  ...
  const handleRefreshView = async () => {
    const campaignId = viewingStatus?.campaign.id;
    if (!campaignId) return;
    setRefreshingCampaign(true);
    try {
      const { data: updated }: { data: CampaignStatusDetail } = await API.get(`/campaigns/${campaignId}/status`);
      const bulk: Record<number, string> = {};
      updated.assets.forEach((a) => { bulk[a.id] = a.approval_status ?? "pending"; });
      setAssetApprovals(bulk);
      setViewingStatus(updated);
      const apSt = updated.campaign.approval_status;
      setViewApproved(apSt === "approved" ? true : apSt === "rejected" ? false : null);
      setCampaigns((prev) => prev.map((c) => c.id === campaignId ? { ...c, ...updated.campaign } : c));
    } catch (err) { console.error(err); }
  ...
  const handleRefreshResults = async () => {
    ...
    try {
      ...
      updated.assets.forEach((a) => { bulk[a.id] = a.approval_status ?? "pending"; });
      setAssetApprovals(bulk);
      setResult((prev) => prev ? { ...prev, status: updated } : prev);
      const apSt = updated.campaign.approval_status;
      setApproved(apSt === "approved" ? true : apSt === "rejected" ? false : null);
      setCampaigns((prev) => prev.map((c) => c.id === campaignId ? { ...c, ...updated.campaign } : c));
    } catch (err) { console.error(err); }
    finally { setRefreshingCampaign(false); }
  };
    ...
    <div className="p-6 max-w-7xl space-y-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {(isView || isResults) && (
            <Button
              onClick={isView ? handleRefreshView : handleRefreshResults}
              variant="outline"
              disabled={refreshingCampaign}
              className="gap-1.5 border-slate-200 text-slate-600 text-sm"
            >
              <RefreshCw className={`w-4 h-4 ${refreshingCampaign ? "animate-spin" : ""}`} />
              {refreshingCampaign ? "Refreshing…" : "Refresh"}
            </Button>
          )}
          {isList && (
          ...
          {loadingView ? (
            ...
            return (
              <>
                ...
                {/* Campaign Send Panel — email and linkedin_social only; linkedin outreach stops at approval */}
                {viewType !== "linkedin" && (DRAFT_EXISTS_STATES.has(c.orchestration_state ?? "") || isContentLocked) && (
                  <CampaignSendPanel
                    campaignId={c.id}
                    campaignType={viewType}
      ...
      {isResults && result && (
        <>
          ...
          <CampaignResultsBody
            ...
            approved={approved}
          />
          {/* Send panel — email and linkedin_social only; linkedin outreach stops at approval */}
          {campaignType !== "linkedin" && DRAFT_EXISTS_STATES.has((result.status.campaign.orchestration_state as string) ?? "") && (
            <CampaignSendPanel
              campaignId={result.campaign_id}
