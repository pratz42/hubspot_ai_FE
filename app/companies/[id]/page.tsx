...
import {
  ...
  Activity, TrendingUp, Clock, Star, Users, DollarSign,
} from "lucide-react";
...
export default function CompanyDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      API.get("/deals", { params: { company_id: id, per_page: 50 } }),
    ])
      .then(([companyRes, contactsRes, leadsRes, dealsRes]) => {
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            ...
            {deals.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-slate-400">No deals associated with this company yet.</p>
              </div>
            ) : (
              <div className="space-y-2">
                {deals.map((deal) => (
